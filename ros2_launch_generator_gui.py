...
class ROS2Command:
    ...
    def __str__(self):
        """Return string representation of the ROS2 command."""
...
class ROS2CommandMonitor:
    ...
    def __init__(self):
        """Initialize the ROS2 command monitor."""
    ...
    def parse_ros2_command(self, pid: int, cmdline: List[str]) -> Optional[ROS2Command]:
        ...
        try:
            ...
            command = ROS2Command(
                pid=pid, timestamp=timestamp, cmd_type=cmd_type, package=package
            )
            ...
            if cmd_type == "run":
                ...
                (
                    ...
                    command.parameters,
                    command.namespace,
                ) = self.parse_arguments(remaining_args)
...
class LaunchFileGenerator:
    ...
    def format_generated_code(code: str) -> str:
        """Format generated code using black and ruff for professional quality."""
        # Step 1: Format with Black (code formatting)
        try:
            import black
            ...
            print("⚠️  Black not available, using basic formatting")
            formatted_code = LaunchFileGenerator._basic_format(code)
        except Exception as e:
            print(f"⚠️  Warning: Could not format code with black: {e}")
            formatted_code = LaunchFileGenerator._basic_format(code)

        # Step 2: Apply Ruff fixes (linting and additional formatting)
        try:
            formatted_code = LaunchFileGenerator._apply_ruff_fixes(formatted_code)
            print("✅ Applied Ruff linting fixes")
        except ImportError:
            print("⚠️  Ruff not available, skipping additional linting")
        except Exception as e:
            print(f"⚠️  Warning: Could not apply ruff fixes: {e}")

        return formatted_code

    @staticmethod
    def _apply_ruff_fixes(code: str) -> str:
        ...
        try:
            # Run ruff with fix and format options
            subprocess.run(
                ["ruff", "check", "--fix", "--silent", temp_path],
                capture_output=True,
                text=True,
            )

            # Apply ruff format (newer versions)
            try:
                subprocess.run(
                    ["ruff", "format", "--silent", temp_path],
                    capture_output=True,
                    text=True,
                    check=True,
                )
            except (subprocess.CalledProcessError, FileNotFoundError):
                # Fallback for older ruff versions or if format command is not available
                pass
    ...
    def _basic_format(code: str) -> str:
        """Enhanced basic code formatting when black/ruff are not available."""
        lines = code.split("\n")
        formatted_lines = []
        ...
        for line in lines:
            ...
            if stripped:
                ...
                new_indent = (original_indent // 4) * 4
                ...
                # Handle special cases for Python syntax
                formatted_line = formatted_line.replace(
                    " : = ", ":="
                )  # ROS2 parameter syntax
                formatted_line = formatted_line.replace(
                    " = = ", "=="
                )  # Comparison operator

                formatted_lines.append(" " * new_indent + formatted_line)
            else:
    ...
    def generate_launch_file(
        commands: List[ROS2Command], package_name: str = "generated_launch"
    ) -> str:
        """Generate launch file content from ROS2 commands."""
        launch_content = '''import os
...
'''
        ...
        run_commands = [
            cmd for cmd in commands if cmd.cmd_type == "run" and cmd.selected
        ]
        launch_commands = [
            cmd for cmd in commands if cmd.cmd_type == "launch" and cmd.selected
        ]
...
{','.join(args_list)}
    ...
    return LaunchDescription(declared_arguments + nodes_and_processes)."""
        ...
        # Format the generated code for professional quality
        formatted_content = LaunchFileGenerator.format_generated_code(launch_content)
...
class ROS2LaunchGeneratorGUI:
    ...
    def __init__(self, root):
        """Initialize the ROS2 Launch Generator GUI."""
        self.root = root
    ...
    def setup_gui(self):
        ...
        title_label = ttk.Label(
            main_frame, text="ROS2 Launch File Generator", font=("Arial", 16, "bold")
        )
        ...
        selection_label = ttk.Label(
            control_frame, textvariable=self.selection_var, font=("Arial", 9)
        )
        selection_label.grid(row=1, column=0, columnspan=2, pady=(0, 10))

        # Control buttons
        ttk.Button(control_frame, text="Clear All", command=self.clear_commands).grid(
            row=2, column=0, sticky=tk.W + tk.E, padx=(0, 5)
        )
        ttk.Button(control_frame, text="Select All", command=self.select_all).grid(
            row=2, column=1, sticky=tk.W + tk.E, padx=(5, 0)
        )

        ttk.Button(control_frame, text="Deselect All", command=self.deselect_all).grid(
            row=3, column=0, sticky=tk.W + tk.E, padx=(0, 5), pady=(5, 0)
        )
        ttk.Button(
            control_frame, text="Manual Refresh", command=self.force_refresh
        ).grid(row=3, column=1, sticky=tk.W + tk.E, padx=(5, 0), pady=(5, 0))

        # Generate button
        ttk.Button(
            control_frame,
            text="Generate Launch File",
            command=self.generate_launch_file,
            style="Accent.TButton",
        ).grid(row=4, column=0, columnspan=2, sticky=tk.W + tk.E, pady=(10, 0))

        # Instructions
        instructions_text = "Instructions:\n• Double-click to toggle selection\n• Enter/Space to toggle selected row\n• Monitor detects ROS2 commands automatically"
        ttk.Label(
            control_frame, text=instructions_text, font=("Arial", 8), foreground="gray"
        ).grid(row=5, column=0, columnspan=2, pady=(10, 0), sticky=tk.W)

        # Commands list frame
        commands_frame = ttk.LabelFrame(
            main_frame, text="Monitored ROS2 Commands", padding="5"
        )
        ...
        self.tree = ttk.Treeview(
            commands_frame, columns=columns, show="headings", height=15
        )
        ...
        v_scrollbar = ttk.Scrollbar(
            commands_frame, orient=tk.VERTICAL, command=self.tree.yview
        )
        h_scrollbar = ttk.Scrollbar(
            commands_frame, orient=tk.HORIZONTAL, command=self.tree.xview
        )
        self.tree.configure(
        ...
        preview_frame = ttk.LabelFrame(
            main_frame, text="Launch File Preview", padding="5"
        )
        preview_frame.grid(
        ...
        self.preview_text = scrolledtext.ScrolledText(
            preview_frame, width=50, height=20, font=("Courier", 9)
        )
        self.preview_text.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))

        # Save button for preview
        ttk.Button(
            preview_frame, text="Save Launch File", command=self.save_launch_file
    ...
    def refresh_commands(self):
        ...
        if len(commands) == self.last_command_count and hasattr(
            self, "_last_refresh_forced"
        ):
        ...
        self.selection_var.set(
            f"Commands: {total_commands} | Selected: {selected_commands}"
        )
            ...
            try:
                ...
                if len(values) > 1:
                    ...
                    if pid in self.monitor.commands:
                        ...
                        self.refresh_commands()
                        print(
                            f"Toggled selection for PID {pid}: {'Selected' if cmd.selected else 'Deselected'}"
                        )
    ...
    def update_preview(self):
        ...
        if selected_commands:
            preview_content = LaunchFileGenerator.generate_launch_file(
                selected_commands
            )
        else:
    ...
    def generate_launch_file(self):
        ...
        if not selected_commands:
            messagebox.showwarning(
                "No Selection",
                "Please select at least one ROS2 command to generate a launch file.",
            )
            return
        ...
        messagebox.showinfo(
            "Success",
            f"Launch file generated with {len(selected_commands)} command(s)!",
        )
